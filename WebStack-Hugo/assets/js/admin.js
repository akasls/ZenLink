    ...
    $(".sortable").sortable({
        ...
        update: function(event, ui) {
            ...
            $.each(sortedIDs, function(index, value) {
              formData.append('data[' + index + '][]', value.slice(1));
    ...
    card_menu = function() {
    	dropdown.render({
    		...
    		click: function(data, othis) {
    		    var elem    = $(this.elem);
    		    var id = elem.attr('id').slice(1);
    		    var url = $('#'+ elem.attr('id') + ' a').attr("href");
    		    var type = url.includes("article") ? 'wz' : 'link';
    			...
    			switch(data.id) {
    				...
    				case 2: //复制
    				    if(isSupported){
    				        url = url.substr(0, 2) == "./" ? baseUrl + url.slice(2) : url;
    				...
    				case 3: //编辑
    				    ...
    				    if(type == "wz"){
    				        layer.open({
                                ...
                                shadeClose: true,
                                scrollbar: false,
                                content: `./index.php?c=admin&page=expand/article-edit&id=${id}&u=${u}&source=tpl`,
                                area: [ '100%', '100%']
                            });
    				    }else if(type == "link"){
    				        openLayer('./index.php?c=admin&source=tpl&page=link_edit&u=' + u +'&id=' + id,'编辑链接');
    				    }else{
    				        layer.msg('不支持操作的类型', {icon: 5});
    				    }
    					break;
...
//删除文章
function deleteWZ(id,elem) {
	$.post(get_api('write_article','del_article'), {
		id: `[${id}]`
	}, function(data, status) {
		if(data.code == 1) {
			layer.msg('删除成功！',{icon: 6,time: 2000});
